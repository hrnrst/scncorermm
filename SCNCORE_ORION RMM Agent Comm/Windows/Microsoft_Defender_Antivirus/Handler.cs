...
namespace Windows.Microsoft_Defender_Antivirus
{
    internal class Handler
    {
        ...
        public static async void Events_Tick(object source, ElapsedEventArgs e)
        {
            ...
            // SCNCORE_ORION legacy code. Will be worked in future

            //Only do if sensor management ruleset is assigned
            /*if (SCNCORE_ORION_Agent_Service.sensor_management_ruleset != "LQ==")
            {
                ...
                if (SCNCORE_ORION_Agent_Service.microsoft_defender_antivirus_crawling)
                {
                    Logging.Logging.incident_response("Microsoft_Defender_AntiVirus.Handler.MSDAV_Events_Tick", "status", "already crawling, abort");
                    return;
                }

                SCNCORE_ORION_Agent_Service.microsoft_defender_antivirus_crawling = true;

                Eventlog_Crawler.Eventlog_Crawler.EDR();

                SCNCORE_ORION_Agent_Service.microsoft_defender_antivirus_crawling = false;
            }
            else
        ...
        public static string Get_Notifications_Json()
        {
            try
            {
                bool notifications_SCNCORE_ORION_mail = false;
                bool notifications_SCNCORE_ORION_microsoft_teams = false;
                bool notifications_SCNCORE_ORION_telegram = false;
                bool notifications_SCNCORE_ORION_ntfy_sh = false;

                using (JsonDocument document = JsonDocument.Parse(Windows_Worker.policy_antivirus_settings_json))
                {
                    JsonElement notifications_SCNCORE_ORION_mail_element = document.RootElement.GetProperty("notifications_SCNCORE_ORION_mail");
                    notifications_SCNCORE_ORION_mail = notifications_SCNCORE_ORION_mail_element.GetBoolean();

                    JsonElement notifications_SCNCORE_ORION_microsoft_teams_element = document.RootElement.GetProperty("notifications_SCNCORE_ORION_microsoft_teams");
                    notifications_SCNCORE_ORION_microsoft_teams = notifications_SCNCORE_ORION_microsoft_teams_element.GetBoolean();

                    JsonElement notifications_SCNCORE_ORION_telegram_element = document.RootElement.GetProperty("notifications_SCNCORE_ORION_telegram");
                    notifications_SCNCORE_ORION_telegram = notifications_SCNCORE_ORION_telegram_element.GetBoolean();

                    JsonElement notifications_SCNCORE_ORION_ntfy_sh_element = document.RootElement.GetProperty("notifications_SCNCORE_ORION_ntfy_sh");
                    notifications_SCNCORE_ORION_ntfy_sh = notifications_SCNCORE_ORION_ntfy_sh_element.GetBoolean();
                }

                // Create notifications_json
                string notifications_json = "{\"mail\":" + notifications_SCNCORE_ORION_mail.ToString().ToLower() + ",\"microsoft_teams\":" + notifications_SCNCORE_ORION_microsoft_teams.ToString().ToLower() + ",\"telegram\":" + notifications_SCNCORE_ORION_telegram.ToString().ToLower() + ",\"ntfy_sh\":" + notifications_SCNCORE_ORION_ntfy_sh.ToString().ToLower() + "}";
