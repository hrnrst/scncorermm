    ...
    internal class Logger
    {
        ...
        public static void Consume_Events()
        {
            ...
            try
            {
                ...
                using (SqliteConnection db_conn = new SqliteConnection(Application_Settings.SCNCORE_ORION_Events_Database_String))
                {
                    db_conn.Open();
                    ...
                    foreach (DataRow row in Device_Worker.events_data_table.Rows)
                    {
                        SqliteCommand command = new SqliteCommand("INSERT INTO events ('severity', 'reported_by', 'event', 'description', 'notification_json', 'type', 'language', 'status') VALUES (" +
                            "'" + Encryption.String_Encryption.Encrypt(row["severity"].ToString(), Application_Settings.SCNCORE_ORION_Local_Encryption_Key) + "', " + //_event
                            "'" + Encryption.String_Encryption.Encrypt(row["reported_by"].ToString(), Application_Settings.SCNCORE_ORION_Local_Encryption_Key) + "', " + //_event
                            "'" + Encryption.String_Encryption.Encrypt(row["event"].ToString(), Application_Settings.SCNCORE_ORION_Local_Encryption_Key) + "', " + //_event
                            "'" + Encryption.String_Encryption.Encrypt(row["description"].ToString(), Application_Settings.SCNCORE_ORION_Local_Encryption_Key) + "', " + //description
                            "'" + Encryption.String_Encryption.Encrypt(row["notification_json"].ToString(), Application_Settings.SCNCORE_ORION_Local_Encryption_Key) + "', " + //notification_json
                            "'" + Encryption.String_Encryption.Encrypt(row["type"].ToString(), Application_Settings.SCNCORE_ORION_Local_Encryption_Key) + "', " + //type
                            "'" + Encryption.String_Encryption.Encrypt(row["language"].ToString(), Application_Settings.SCNCORE_ORION_Local_Encryption_Key) + "', " + //language
                            "'" + Encryption.String_Encryption.Encrypt("0", Application_Settings.SCNCORE_ORION_Local_Encryption_Key) + "'" + //status
                            ")"
                            , db_conn);
            ...
            try
            {
                ...
                using (SqliteConnection db_conn = new SqliteConnection(Application_Settings.SCNCORE_ORION_Events_Database_String))
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        if (Encryption.String_Encryption.Decrypt(reader["status"].ToString(), Application_Settings.SCNCORE_ORION_Local_Encryption_Key) == "0")
                            status_send = await Events.Sender.Send_Event(Encryption.String_Encryption.Decrypt(reader["severity"].ToString(), Application_Settings.SCNCORE_ORION_Local_Encryption_Key), Encryption.String_Encryption.Decrypt(reader["reported_by"].ToString(), Application_Settings.SCNCORE_ORION_Local_Encryption_Key), Encryption.String_Encryption.Decrypt(reader["event"].ToString(), Application_Settings.SCNCORE_ORION_Local_Encryption_Key), Encryption.String_Encryption.Decrypt(reader["description"].ToString(), Application_Settings.SCNCORE_ORION_Local_Encryption_Key), Encryption.String_Encryption.Decrypt(reader["notification_json"].ToString(), Application_Settings.SCNCORE_ORION_Local_Encryption_Key), Encryption.String_Encryption.Decrypt(reader["type"].ToString(), Application_Settings.SCNCORE_ORION_Local_Encryption_Key), Encryption.String_Encryption.Decrypt(reader["language"].ToString(), Application_Settings.SCNCORE_ORION_Local_Encryption_Key));

                        //If status send success, update its status in db
                        if (status_send)
                        {
                            SqliteCommand command = new SqliteCommand("UPDATE events SET status = '" + Encryption.String_Encryption.Encrypt("1", Application_Settings.SCNCORE_ORION_Local_Encryption_Key) + "' WHERE id = '" + reader["id"].ToString() + "';", db_conn);
                            command.ExecuteNonQuery();
                        }
                        ...
                        if (Encryption.String_Encryption.Decrypt(reader["status"].ToString(), Application_Settings.SCNCORE_ORION_Local_Encryption_Key) == "1")
                            log_delete = true;
