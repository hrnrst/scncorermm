    ...
    public class Microsoft_Defender_Firewall
    {
        ...
        public static void Rule_Inbound(string port)
        {
            try
            {
                ...
                bool rule_existing = firewallPolicy.Rules.Cast<INetFwRule>().Any(rule => rule.Name == "SCNCORE_ORION RMM Web Console Inbound HTTP (" + port + ")");
                ...
                if (!rule_existing)
                {
                    ...
                    new_rule.Name = "SCNCORE_ORION RMM Web Console Inbound HTTP (" + port + ")";
            ...
            {
                Logging.Handler.Error("Microsoft_Defender_Firewall.Rule_Inbound", "Add SCNCORE_ORION service rule (outbound)", ex.ToString());
            }
        }
        ...
        public static void Rule_Outbound(string port)
        {
            try
            {
                ...
                // Check if SCNCORE_ORION service rule is existing
                bool rule_existing = firewallPolicy.Rules.Cast<INetFwRule>().Any(rule => rule.Name == "SCNCORE_ORION RMM Web Console Outbound HTTPS (" + port + ")");
                ...
                if (!rule_existing)
                {
                    ...
                    INetFwRule2 new_rule = (INetFwRule2)Activator.CreateInstance(Type.GetTypeFromProgID("HNetCfg.FWRule"));
                    new_rule.Name = "SCNCORE_ORION RMM Web Console Outbound HTTPS (" + port + ")";
                    new_rule.Enabled = true;
            ...
            {
                Logging.Handler.Error("Microsoft_Defender_Firewall.Rule_Outbound", "Add SCNCORE_ORION service rule (outbound)", ex.ToString());
            }
        }
    }
}
