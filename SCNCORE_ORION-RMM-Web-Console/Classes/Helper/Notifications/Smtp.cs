...
namespace SCNCORE_ORION_RMM_Web_Console.Classes.Helper.Notifications
{
    public class Smtp
    {
            ...
            try
            {
                using (SmtpClient smtpClient = new SmtpClient(smtpSettings.server, Convert.ToInt32(smtpSettings.port)))
                {
                    ...
                    using (MailMessage mailMessage = new MailMessage())
                    {
                        ...
                        mailMessage.From = new MailAddress("Alerts | SCNCORE_ORION <" + smtpSettings.username + ">");
        ...
        public static async Task<string> Test_Smtp(string username, string password, string server, int port, bool ssl, string from)
        {
            try
            {
                ...
                using (SmtpClient smtpClient = new SmtpClient(server, port))
                {
                    ...
                    {
                        // Setze Absender, Empfänger, Betreff und Nachrichtentext
                        mailMessage.From = new MailAddress(from, "Alerts | SCNCORE_ORION");
                        mailMessage.To.Add(username); // Füge hier die Empfänger-E-Mail-Adresse hinzu
                        mailMessage.Subject = "SCNCORE_ORION - Test Alert";
