    ...
    internal class Handler
    {
        ...
        public static void Rule_Inbound(string port)
        {
            try
            {
                ...
                bool rule_existing = firewallPolicy.Rules.Cast<INetFwRule>().Any(rule => rule.Name == "SCNCORE_ORION RMM Server Inbound HTTP (" + port + ")");
                ...
                if (!rule_existing)
                {
                    ...
                    new_rule.Name = "SCNCORE_ORION RMM Server Inbound HTTP (" + port + ")";
        ...
        public static void Rule_Outbound(string port)
        {
            try
            {
                ...
                // Check if SCNCORE_ORION service rule is existing
                bool rule_existing = firewallPolicy.Rules.Cast<INetFwRule>().Any(rule => rule.Name == "SCNCORE_ORION RMM Server Outbound HTTPS (" + port + ")");
                ...
                if (!rule_existing)
                {
                    ...
                    INetFwRule2 new_rule = (INetFwRule2)Activator.CreateInstance(Type.GetTypeFromProgID("HNetCfg.FWRule"));
                    new_rule.Name = "SCNCORE_ORION RMM Server Outbound HTTPS (" + port + ")";
                    new_rule.Enabled = true;
