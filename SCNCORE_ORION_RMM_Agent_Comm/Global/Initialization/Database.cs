...
namespace Global.Initialization
{
    ...
    {
        public static void SCNCORE_ORION_Data_Setup()
        {
            string data_db_path = Application_Paths.program_data_SCNCORE_ORION_policy_database;
            ...
            try
            {
                ...
                while (File.Exists(data_db_path))
                {
                    try
                    {
                        ...
                        Logging.Debug("Initialization.Database.SCNCORE_ORION_Data_Setup", "check_db", "DB deleted.");
                    }
                    catch (Exception ex)
                    {
                        Logging.Error("Initialization.Database.SCNCORE_ORION_Data_Setup", "check_db", "Failed deleting DB: " + ex.ToString());
                    }
                ...
                */
                ...
                if (File.Exists(data_db_path))
                {
                    ...
                    using (var conn = new SqliteConnection(Application_Settings.SCNCORE_ORION_Data_Database_String))
                    {
                        conn.Open();
                    ...
                    Logging.Debug("Initialization.Database.SCNCORE_ORION_Data_Setup", "check_db", "All tables dropped.");
                    ...
                    Logging.Debug("Initialization.Database.SCNCORE_ORION_Data_Setup", "check_db", "Database file shrunk.");
                ...
                try
                {
                    ...
                    {
                        Logging.Debug("Initialization.Database.SCNCORE_ORION_Data_Setup", "create_db (data)", "DB not existing, creating new one.");

                        // Create the database file
                        using (SqliteConnection db_conn = new SqliteConnection(Application_Settings.SCNCORE_ORION_Data_Database_String))
                        {
                            db_conn.Open();
                            db_conn.Close();
                            db_conn.Dispose();
                        ...
                        Logging.Debug("Initialization.Database.SCNCORE_ORION_Data_Setup", "create_db (data)", "DB created.");
                    }
                    else
                        Logging.Debug("Initialization.Database.SCNCORE_ORION_Data_Setup", "create_db (data)", "DB still existing.");
                }
                catch (Exception ex)
                {
                    Logging.Error("Initialization.Database.SCNCORE_ORION_Data_Setup", "create_db (data)", ex.ToString());
                }
                ...
                try
                {
                    using (SqliteConnection db_conn = new SqliteConnection(Application_Settings.SCNCORE_ORION_Data_Database_String))
                    {
                        ...
                        */
                        ...
                        Logging.Debug("Initialization.Database.SCNCORE_ORION_Data_Setup", "check_db", "Filled DB info.");
                    }
                }
                catch (Exception ex)
                {
                    Logging.Error("Initialization.Database.SCNCORE_ORION_Data_Setup", "create_application_control_ruleset", ex.ToString());
                    SCNCORE_ORION_Data_Setup();
                }
            }
            catch (Exception ex)
            {
                Logging.Error("Initialization.Database.SCNCORE_ORION_Data_Setup", "check_db", ex.ToString());
                SCNCORE_ORION_Data_Setup();
            }
        }

        public static void SCNCORE_ORION_Events_Setup()
        {
            GC.Collect();
            GC.WaitForPendingFinalizers();

            string report_db_path = Application_Paths.program_data_SCNCORE_ORION_events_database;
            ...
            try
            {
                if (!File.Exists(report_db_path))
                {
                    ...
                    using (SqliteConnection db_conn = new SqliteConnection(Application_Settings.SCNCORE_ORION_Events_Database_String))
                    {
                        ...
                        Logging.Debug("Initialization.Database.SCNCORE_ORION_Events_Setup", "check_db (events)", "Created events table.");
                    }
                }
                else
                    Logging.Debug("Initialization.Database.SCNCORE_ORION_Events_Setup", "create_db (events)", "Rep DB still existing.");
            }
            catch (Exception ex)
            {
                Logging.Error("Initialization.Database.SCNCORE_ORION_Events_Setup", "create_db general (events)", ex.ToString());
            }
        }
    }
}
