    ...
    internal class Health
    {
        ...
        { 
            try
            {
                ...
                if (!Windows.Helper.Registry.HKLM_Key_Exists(Application_Paths.SCNCORE_ORION_reg_path))
                    Windows.Helper.Registry.HKLM_Create_Key(Application_Paths.SCNCORE_ORION_reg_path);

                // Check if msdav key exists
                if (!Windows.Helper.Registry.HKLM_Key_Exists(Application_Paths.SCNCORE_ORION_microsoft_defender_antivirus_reg_path))
                    Windows.Helper.Registry.HKLM_Create_Key(Application_Paths.SCNCORE_ORION_microsoft_defender_antivirus_reg_path);
            }
            catch (Exception ex)
            {
        ...
        {
            Windows.Microsoft_Defender_Firewall.Handler.SCNCORE_ORION_RMM_Comm_Agent_Rule_Inbound();
            Windows.Microsoft_Defender_Firewall.Handler.SCNCORE_ORION_RMM_Comm_Agent_Rule_Outbound();
            Windows.Microsoft_Defender_Firewall.Handler.SCNCORE_ORION_RMM_Health_Service_Rule();
            Windows.Microsoft_Defender_Firewall.Handler.SCNCORE_ORION_Installer_Rule();
            Windows.Microsoft_Defender_Firewall.Handler.SCNCORE_ORION_Uninstaller_Rule();
        }*/
        ...
        {
            // Check if the databases are in place
            if (!File.Exists(Application_Paths.program_data_SCNCORE_ORION_policy_database))
                Database.SCNCORE_ORION_Data_Setup();

            // Check if the events database is in place
            if (!File.Exists(Application_Paths.program_data_SCNCORE_ORION_events_database))
                Database.SCNCORE_ORION_Events_Setup();
        }
