                ...
                using (var httpClient = new HttpClient())
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        if (result != "no_assigned_policy_found")
                        {
                            ...
                            Initialization.Database.SCNCORE_ORION_Data_Setup();

                            Logging.Debug("Online_Mode.Handler.Policy", "Insert into policy database", "Starting...");

                            using (SqliteConnection db_conn = new SqliteConnection(Application_Settings.SCNCORE_ORION_Data_Database_String)) // Remove old policy and insert new policy
                            {
                                db_conn.Open();
