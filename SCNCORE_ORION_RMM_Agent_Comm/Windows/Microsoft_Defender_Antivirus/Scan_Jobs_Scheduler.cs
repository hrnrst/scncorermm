...
//using SCNCORE_ORION_RMM_Comm_Agent_Windows.Events;
                ...
                foreach (var job in Directory.GetFiles(Application_Paths.program_data_microsoft_defender_antivirus_scan_jobs))
                {
                    ...
                    if (execute)
                    {
                        ...
                        if (Global.Configuration.Agent.language == "en-US")
                            Global.Events.Logger.Insert_Event("0", "Microsoft Defender Antivirus", "Scan job completed. SCNCORE_ORION report.", "Scan job: " + job_item.name + " (" + job_item.description + ") " + Environment.NewLine + Environment.NewLine + "Result: " + "Cannot be retrieved. We are currently not aware of any way to directly determine the result of a scan job executed with PowerShell. Do you have an idea? Feel free to contact us.", Windows_Worker.microsoft_defender_antivirus_notifications_json, 0, 0);
                        else if (Global.Configuration.Agent.language == "de-DE")
                            Global.Events.Logger.Insert_Event("0", "Microsoft Defender Antivirus", "Scanauftrag fertiggestellt. SCNCORE_ORION Bericht.", "Scan job: " + job_item.name + " (" + job_item.description + ") " + Environment.NewLine + Environment.NewLine + "Ergebnis: " + "Kann nicht abgerufen werden. Aktuell ist uns kein Weg bekannt, um das Ergebnis eines mit PowerShell ausgeführten Scanauftrags direkt zu ermitteln. Hast du eine Idee? Kontaktiere uns gerne.", Windows_Worker.microsoft_defender_antivirus_notifications_json, 0, 1);
