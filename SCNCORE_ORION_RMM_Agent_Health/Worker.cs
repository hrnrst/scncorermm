    ...
    public class Worker : BackgroundService
    {
        ...
        private async void Check_Health()
        {
            ...
            try
            {
                ...
                {
                    ServiceController sc = new ServiceController("SCNCORE_ORION_RMM_Agent_Comm");
                    if (sc.Status.Equals(ServiceControllerStatus.StartPending) || sc.Status.Equals(ServiceControllerStatus.Running))
                    {
                        Logging.Debug("Check_Health (SCNCORE_ORION_RMM_Agent_Comm)", "Running", "Installed & running.");
                    ...
                    {
                        Logging.Debug("Check_Health (SCNCORE_ORION_RMM_Agent_Comm)", "Stopped|StopPending", "Installed, but stopped or stop is pending.");
                    }

                    sc = new ServiceController("SCNCORE_ORION_RMM_Agent_Remote");
                    if (sc.Status.Equals(ServiceControllerStatus.StartPending) || sc.Status.Equals(ServiceControllerStatus.Running))
                    {
                        Logging.Debug("Check_Health (SCNCORE_ORION_RMM_Agent_Remote)", "Running", "Installed & running.");
                        remote_agent_healthy = true;
                    }
                    else if (sc.Status.Equals(ServiceControllerStatus.Stopped) || sc.Status.Equals(ServiceControllerStatus.StopPending))
                    {
                        Logging.Debug("Check_Health (SCNCORE_ORION_RMM_Agent_Remote)", "Stopped|StopPending", "Installed, but stopped or stop is pending.");
                    }
                }
                else if (OperatingSystem.IsLinux())
                {
                    string[] servicesToCheck = { "scncore_orion-rmm-agent-comm", "scncore_orion-rmm-agent-remote" };
                    ...
                    foreach (var serviceName in servicesToCheck)
                    {
                        ...
                        if (isServiceRunning)
                        {
                            ...
                            if (serviceName == "scncore_orion-rmm-agent-comm")
                                comm_agent_healthy = true;
                            else if (serviceName == "scncore_orion-rmm-agent-remote")
                                remote_agent_healthy = true;
                        }
                        else
                        {
            ...
            if (!comm_agent_healthy || !remote_agent_healthy)
            {
                ...
                try
                {
                    if (OperatingSystem.IsWindows())
                    {
                        ...
                        {
                            sc = new ServiceController("scncore_orion_RMM_Agent_Comm");
                            sc.Start();
                            failed_count = 0;
                    ...
                    else if (OperatingSystem.IsLinux())
                    {
                        ...
                        {
                            // Start the service
                            Bash.Execute_Script("Service Start", false, "systemctl start scncore_orion-rmm-agent-comm.service");

                            // Check status
                            string output = Bash.Execute_Script("Service status", false, "systemctl list-units --type=service --all | grep -w scncore_orion-rmm-agent-comm.service");
                        ...
                        {
                            // Start the service
                            Bash.Execute_Script("Service Start", false, "systemctl start scncore_orion-rmm-agent-remote.service");
                            
                            // Check status
                            string output = Bash.Execute_Script("Service Sensor", false, "systemctl list-units --type=service --all | grep -w scncore_orion-rmm-agent-remote.service");
                    ...
                    else if (OperatingSystem.IsMacOS())
                    {
                        ...
                        {
                            // Start the service
                            MacOS.Helper.Zsh.Execute_Script("Service Start", false, "launchctl start scncore_orion-rmm-agent-comm");

                            string output = MacOS.Helper.Zsh.Execute_Script("Service Sensor", false, $"launchctl list | grep scncore_orion-rmm-agent-comm");

                            Logging.Debug("Check_Health", "Service status output", output);

                            // Regex to extract only the line for the specific service
                            string pattern = $@"^\S+\s+\S+\s+scncore_orion-rmm-agent-comm$";
                        ...
                        {
                            // Start the service
                            MacOS.Helper.Zsh.Execute_Script("Service Start", false, "launchctl start scncore_orion-rmm-agent-remote");
                         
                            string output = MacOS.Helper.Zsh.Execute_Script("Service Sensor", false, $"launchctl list | grep scncore_orion-rmm-agent-remote");

                            Logging.Debug("Check_Health", "Service status output", output);

                            // Regex to extract only the line for the specific service
                            string pattern = $@"^\S+\s+\S+\s+scncore_orion-rmm-agent-remote$";
                            var match = Regex.Match(output, pattern, RegexOptions.Multiline);
            ...
            if (failed_count == 3 || failed_count > 3)
            {
                ...
                if (!Directory.Exists(Application_Paths.c_temp_scncore_orion_installer_dir))
                    Directory.CreateDirectory(Application_Paths.c_temp_scncore_orion_installer_dir);
                ...
                http_status = await Http.DownloadFileAsync(Global.Configuration.Agent.ssl, update_server + installer_package_url, Path.Combine(Application_Paths.c_temp_scncore_orion_dir, Application_Paths.installer_package_path), Global.Configuration.Agent.package_guid);
                ...
                if (http_status && !String.IsNullOrEmpty(installer_hash) && IO.Get_SHA512(Path.Combine(Application_Paths.c_temp_scncore_orion_dir, Application_Paths.installer_package_path)) == installer_hash)
                {
                    ...
                    try
                    {
                        ZipFile.ExtractToDirectory(Path.Combine(Application_Paths.c_temp_scncore_orion_dir, Application_Paths.installer_package_path), Application_Paths.c_temp_scncore_orion_installer_dir);
                        ...
                        if (OperatingSystem.IsWindows())
                        {
                            ...
                            installer.StartInfo.FileName = Application_Paths.c_temp_scncore_orion_installer_path;
                        ...
                        else if (OperatingSystem.IsLinux())
                        {
                            ...
                            installer.StartInfo.ArgumentList.Add($"systemctl start scncore_orion-rmm-agent-installer");
                        ...
                        else if (OperatingSystem.IsMacOS())
                        {
                            ...
                            installer.StartInfo.ArgumentList.Add("-c");
                            installer.StartInfo.ArgumentList.Add($"sudo launchctl load -w /Library/LaunchDaemons/com.scncore_orion.rmm.installer.plist");
                            installer.StartInfo.WindowStyle = ProcessWindowStyle.Hidden;
                            installer.StartInfo.RedirectStandardOutput = true;
                            installer.StartInfo.RedirectStandardError = true;
                            installer.Start();
        ...
        private void Delete_Installer()
        {
            ...
            {
                if (Directory.Exists(Application_Paths.c_temp_scncore_orion_installer_dir))
                    Directory.Delete(Application_Paths.c_temp_scncore_orion_installer_dir, true);
            }
            catch (Exception ex)
            {
