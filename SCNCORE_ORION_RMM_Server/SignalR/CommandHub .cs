...
namespace SCNCORE_ORION_RMM_Server.SignalR
{
        ...
        public async Task MessageReceivedFromWebconsole(string message)
        {
            try
            {
                ...
                if (String.IsNullOrEmpty(client_id) && command.type == 0) // if remote shell
                {
                    ...
                    await Clients.Caller.SendAsync("ReceiveClientResponseRemoteShell", "Remote device is not connected with the SCNCORE_ORION RMM backend. Make sure your target device is connected.");
                ...
                {
                    Logging.Handler.Debug("SignalR CommandHub", "MessageReceivedFromWebconsole", "Client ID not found.");
                    await Clients.Caller.SendAsync("ReceiveClientResponseRemoteControl", "Remote device is not connected with the SCNCORE_ORION RMM backend. Make sure your target device is connected.");
                    return;
                }
                else if (command.type == 5) // check connection
                {
                    ...
                    {
                        Logging.Handler.Debug("SignalR CommandHub", "MessageReceivedFromWebconsole", "Client ID not found.");
                        await Clients.Caller.SendAsync("ReceiveClientResponseCheckConnection", "Remote device is not connected with the SCNCORE_ORION RMM backend. Make sure your target device is connected.");
                    ...
                    else
                    {
                        ...
                        await Clients.Caller.SendAsync("ReceiveClientResponseCheckConnection", "Remote device is connected with the SCNCORE_ORION RMM backend.");
                    }
                }
                else if (String.IsNullOrEmpty(client_id))
                {
                    Logging.Handler.Debug("SignalR CommandHub", "MessageReceivedFromWebconsole", "Remote device is not connected with the SCNCORE_ORION RMM backend. Make sure your target device is connected.");
                    await Clients.Caller.SendAsync("ReceiveClientResponse", "Remote device is not connected with the SCNCORE_ORION RMM backend. Make sure your target device is connected.");
