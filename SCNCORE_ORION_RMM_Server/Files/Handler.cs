...
using SCNCORE_ORION_RMM_Server.MySQL;
using System;
...
using SCNCORE_ORION_RMM_Server.Files.Compiler;
using System.Diagnostics.Eventing.Reader;
using SCNCORE_ORION_RMM_Server.Configuration;

namespace SCNCORE_ORION_RMM_Server.Files
{
    ...
    public class Handler
    {
        ...
        public static async Task<string> Register_File(string file_path, string tenant_guid, string location_guid, string device_name)
        {
            try
            {
                ...
                (int tenant_id, int location_id) = await SCNCORE_ORION_RMM_Server.Agent.Windows.Helper.Get_Tenant_Location_Id(tenant_guid, location_guid);

                // Get device ID
                int device_id = await SCNCORE_ORION_RMM_Server.Agent.Windows.Helper.Get_Device_Id(device_name, tenant_id, location_id);
        ...
        public static async Task<string> Create_Custom_Installer(string name, string json)
        {
            try
            {
                ...
                if (architecture == "win-x64" || architecture == "win-arm64")
                    installer_executable = Path.Combine(installer_extracted_dir, "SCNCORE_ORION_RMM_Agent_Installer.exe");
                else if (architecture == "linux-x64" || architecture == "linux-arm64")
                    installer_executable = Path.Combine(installer_extracted_dir, "SCNCORE_ORION_RMM_Agent_Installer");
                ...
                string installer_download_location = Path.Combine(Application_Paths._private_files_SCNCORE_ORION_temp, name + "-" + architecture + ".zip");
