...
public class Events_Notification_Service : BackgroundService
{
    ...
    private async Task ProcessEventsTask()
    {
        ...
        try
        {
            ...
            {
                SCNCORE_ORION_RMM_Server.Events.Sender.Smtp("mail_status", "mail_notifications"),
                SCNCORE_ORION_RMM_Server.Events.Sender.Smtp("ms_teams_status", "microsoft_teams_notifications"),
                SCNCORE_ORION_RMM_Server.Events.Sender.Smtp("telegram_status", "telegram_notifications"),
                SCNCORE_ORION_RMM_Server.Events.Sender.Smtp("ntfy_sh_status", "ntfy_sh_notifications")
            };

            await Task.WhenAll(tasks);*/

            await SCNCORE_ORION_RMM_Server.Events.Sender.Smtp("mail_status", "mail_notifications");
            await SCNCORE_ORION_RMM_Server.Events.Sender.Smtp("ms_teams_status", "microsoft_teams_notifications");
            await SCNCORE_ORION_RMM_Server.Events.Sender.Smtp("telegram_status", "telegram_notifications");
            await SCNCORE_ORION_RMM_Server.Events.Sender.Smtp("ntfy_sh_status", "ntfy_sh_notifications");
            ...
            await SCNCORE_ORION_RMM_Server.Events.Sender.Mark_Old_Read(startedTime, finishedTime);
        }
        catch (Exception ex)
        {
