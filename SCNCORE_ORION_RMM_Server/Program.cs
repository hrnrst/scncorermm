...
using SCNCORE_ORION_RMM_Server.Agent.Windows;
using System.Security.Principal;
using Microsoft.AspNetCore.SignalR;
using SCNCORE_ORION_RMM_Server.SignalR;
using System.Net;
using System;
using System.Text.Json;
using static SCNCORE_ORION_RMM_Server.Agent.Windows.Authentification;
using Microsoft.Extensions.DependencyInjection;
using SCNCORE_ORION_RMM_Server;
using SCNCORE_ORION_RMM_Server.Events;
using Microsoft.Extensions.Primitives;
using LettuceEncrypt;
using System.Threading;
using System.IO;
using static SCNCORE_ORION_RMM_Server.SignalR.CommandHub;
using Microsoft.AspNetCore.Builder;
using LLama.Common;
using LLama;
//using SCNCORE_ORION_RMM_Server.LLM;
using Microsoft.AspNetCore.Mvc;
using System.Runtime.InteropServices;
using SCNCORE_ORION_RMM_Server.Configuration;
using System.Security.Cryptography;
using SCNCORE_ORION_RMM_Server.Members_Portal;
...
using SCNCORE_ORION_RMM_Server.Background_Services;

SCNCORE_ORION_RMM_Server.Configuration.Server.serverStartTime = DateTime.Now; // Set server start time

// Check directories
SCNCORE_ORION_RMM_Server.Setup.Directories.Check_Directories(); // Check if directories exist and create them if not
...
// Members Portal Api
var membersPortal = builder.Configuration.GetSection("Members_Portal_Api").Get<SCNCORE_ORION_RMM_Server.Configuration.Members_Portal>() ?? new SCNCORE_ORION_RMM_Server.Configuration.Members_Portal();
...
Console.WriteLine("Server started at: " + SCNCORE_ORION_RMM_Server.Configuration.Server.serverStartTime.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture));
Console.WriteLine(Environment.NewLine);

// Output version
Console.WriteLine("SCNCORE_ORION RMM Server");
...
var mysqlConfig = builder.Configuration.GetSection("MySQL").Get<SCNCORE_ORION_RMM_Server.MySQL.Config>() ?? new SCNCORE_ORION_RMM_Server.MySQL.Config();
...
bool microsoft_defender_firewall_status = SCNCORE_ORION_RMM_Server.Microsoft_Defender_Firewall.Handler.Status();
...
else if (!microsoft_defender_firewall_status && OperatingSystem.IsWindows())
{
    ...
    Console.WriteLine("Microsoft Defender Firewall is disabled. You should enable it for your own safety. SCNCORE_ORION adds firewall rules automatically according to your configuration.");
}

Console.ResetColor();

// Add firewall rule for HTTP
SCNCORE_ORION_RMM_Server.Microsoft_Defender_Firewall.Handler.Rule_Inbound(http_port.ToString());
SCNCORE_ORION_RMM_Server.Microsoft_Defender_Firewall.Handler.Rule_Outbound(http_port.ToString());

if (https)
{
    // Add firewall rule for HTTPS
    SCNCORE_ORION_RMM_Server.Microsoft_Defender_Firewall.Handler.Rule_Inbound(https_port.ToString());
    SCNCORE_ORION_RMM_Server.Microsoft_Defender_Firewall.Handler.Rule_Outbound(https_port.ToString());
...
if (!await SCNCORE_ORION_RMM_Server.MySQL.Handler.Check_Connection())
{
...
else
{
    ...
    if (!await SCNCORE_ORION_RMM_Server.MySQL.Handler.Verify_Supported_SQL_Server())
    {
    ...
    await SCNCORE_ORION_RMM_Server.MySQL.Handler.Update_Server_Information();

    // Get api key
    if (String.IsNullOrEmpty(Members_Portal.api_key))
    {
        Members_Portal.api_key = await SCNCORE_ORION_RMM_Server.MySQL.Handler.Get_Members_Portal_Api_Key();
...
if (role_comm)
{
    app.MapPost("/Agent/Windows/Check_Version", async context =>
    {
        try
        {
            ...
            {
                bool package_guid_status = await Verify_SCNCORE_ORION_Package_Configurations_Guid(package_guid);
...
/*app.MapPost("/Agent/Windows/Remote/Command", async (HttpContext context, IHubContext<CommandHub> hubContext) =>
{
    try
    {
        ...
        bool api_key_status = await SCNCORE_ORION_RMM_Server.SignalR.Webconsole.Handler.Verify_Api_Key(json);
        ...
        // Get the command
        string command = await SCNCORE_ORION_RMM_Server.SignalR.Webconsole.Handler.Get_Command(json);
...
// SCNCORE_ORION admin files, get index
if (role_file)
{
    app.MapPost("/admin/files/index/{path}", async (HttpContext context, string path) =>
    {
        try
        {
            ...
            // Verify API key
            if (!context.Request.Headers.TryGetValue("x-api-key", out StringValues apiKey) || !await SCNCORE_ORION_RMM_Server.Files.Handler.Verify_Api_Key(apiKey))
            {
                context.Response.StatusCode = 401;
                await context.Response.WriteAsync("Unauthorized.");
                return;
...
// SCNCORE_ORION admin files command
if (role_file)
{
    app.MapPost("/admin/files/command", async context =>
    {
        try
        {
            ...
            if (!hasApiKey || !await SCNCORE_ORION_RMM_Server.Files.Handler.Verify_Api_Key(files_api_key))
            {
            ...
            await SCNCORE_ORION_RMM_Server.Files.Handler.Command(json);
...
// SCNCORE_ORION admin files, upload
if (role_file)
{
    app.MapPost("/admin/files/upload/{path}", async (HttpContext context, string path) =>
    {
        try
        {
            ...
            bool ApiKeyValid = await SCNCORE_ORION_RMM_Server.Files.Handler.Verify_Api_Key(files_api_key);
            ...
            string register_json = await SCNCORE_ORION_RMM_Server.Files.Handler.Register_File(filePath, tenant_guid, location_guid, device_name);
...
// SCNCORE_ORION admin files, download
if (role_file)
{
    app.MapGet("/admin/files/download", async (HttpContext context) =>
    {
        try
        {
            ...
            bool hasAccess = await SCNCORE_ORION_RMM_Server.Files.Handler.Verify_File_Access(guid, password, files_api_key); // api key is not required
            ...
            string file_path = await SCNCORE_ORION_RMM_Server.Files.Handler.Get_File_Path_By_GUID(guid);
...
// SCNCORE_ORION admin files device download
app.MapGet("/admin/files/download/device", async (HttpContext context) =>
{
    try
    {
        ...
        if (device_status == "authorized" || device_status == "synced" || device_status == "not_synced")
        {
            ...
            bool file_access = await SCNCORE_ORION_RMM_Server.Files.Handler.Verify_Device_File_Access(tenant_guid, location_guid, device_name, guid);
...
// SCNCORE_ORION admin files device upload
...
// SCNCORE_ORION files download private - GUID, used for update server & trust server
    ...
    app.MapPost("/llm/chat/admin", async (HttpContext context, LLaMaService llamaService) =>
    {
        try
        {
            ...
            if (!context.Request.Headers.TryGetValue("x-api-key", out var apiKey) ||
                !await SCNCORE_ORION_RMM_Server.Files.Handler.Verify_Api_Key(apiKey))
            {
                context.Response.StatusCode = 401;
                await context.Response.WriteAsync("Unauthorized.");
                return;
...
if (role_file)
{
    app.MapPost("/admin/create_installer", async (HttpContext context) =>
    {
        try
        {
            ...
            string result = await SCNCORE_ORION_RMM_Server.Files.Handler.Create_Custom_Installer(name, server_config);
