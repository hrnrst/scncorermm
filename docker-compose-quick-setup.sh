...
known_proxies_json="[]"
IpWhitelist=()
web_console_title="SCNCORE ORION"
apiKeyOverride=""
...
set -e
echo ""
echo "Welcome to the SCNCORE ORION Docker Quick Setup Script!"
...
echo ""
echo "[Requirements]"
echo "[Ubuntu 24.04 or similar]"
echo "[Docker installed]"
echo "[4 GB of free working memory (RAM)]"
echo ""
echo "[ATTENTION!!!]"
echo "The script only covers some scenarios and tries to guide you through the setup process. SCNCORE ORION supports any environment (single IP, reverse proxies etc.). If you fail with your installation, please read the documentation at: https://docs.scncoreorion.com/installation/docker-quick-setup"
echo "If you are a business, need support with your setup and you are interested in the Pro or Cloud membership, please contact us at: support@scncore.com"
echo ""
echo "Our Pro membership includes code signing, unlimited devices, and we assist you with the setup of your SCNCORE ORION instance in your own environment."
echo "We also offer a managed hosting solution for SCNCORE ORION, please contact us for more information or check our website."
...
read -p "Please enter the MySQL root password (will be used for scncoreorion RMM): " mysql_password
...
if [[ -z "$web_console_title" ]]; then
    echo "No title entered. Defaulting to 'SCNCORE ORION'."
    web_console_title="SCNCORE ORION"
fi
...
# Let the user enter their main domain (Please enter your domain you will use for scncoreorion RMM. NOT YOUR SUBDOMAIN!!! Example: scncoreorionrmm.com)
read -p "Please enter the domain (NOT SUBDOMAIN) you will use (example: SCNCORE.COM). The subdomains to be created will be displayed afterwards: " le_domain
if [[ -z "$le_domain" ]]; then
...
if [[ "$setup_option" == "1" ]]; then # No Reverse Proxy with Let's Encrypt
    ...
    echo "7443 for the scncoreorion RMM server (remote access, file transfer, etc.)."
...
# Directories
sudo mkdir -p /home/scncoreorion/{mysql/data,letsencrypt,web_console/internal,web_console/logs,server/logs,server/files,server/internal}

# Create empty appsettings files
sudo touch /home/scncoreorion/web_console/appsettings.json
sudo touch /home/scncoreorion/server/appsettings.json

echo "Creating web console appsettings.json..."

# Write appsettings.json for Web Console (correct path!)
sudo tee /home/scncoreorion/web_console/appsettings.json > /dev/null <<EOF
{
  "Logging": {
    "LogLevel": {
      "Default": "Warning",
  ...
  "scncoreorion_Remote_Server": {
    "Server": "scncoreorion-rmm-server",
    "Port": $web_console_server_port,
    "UseSSL": false
  },
  "scncoreorion_File_Server": {
    "Server": "scncoreorion-rmm-server",
    "Port": $web_console_server_port,
    "UseSSL": false
  },
...
sudo tee /home/scncoreorion/server/appsettings.json > /dev/null <<EOF
{
  "Logging": {
    "LogLevel": {
      "Default": "Warning",
...
sudo touch /home/scncoreorion/docker-compose.yml

sudo tee /home/scncoreorion/docker-compose.yml > /dev/null <<EOF
services:
  mysql:
    image: mysql:8.0
    ...
    environment:
      ...
      MYSQL_DATABASE: scncoreorion
    volumes:
      - /home/scncoreorion/mysql/data:/var/lib/mysql
  ...
  scncoreorion-web-console:
    ...
    environment:
      - TZ=$timezone
    volumes:
      - "/home/scncoreorion/web_console/appsettings.json:/app/appsettings.json"
      - "/home/scncoreorion/web_console/internal:/app/internal"
      - "/home/scncoreorion/web_console/logs:/var/0x101 Cyber Security/scncoreorion RMM/Web Console/"
  ...
  scncoreorion-rmm-server:
    ...
    environment:
      - TZ=$timezone
    volumes:
      - "/home/scncoreorion/server/appsettings.json:/app/appsettings.json"
      - "/home/scncoreorion/server/internal:/app/internal"
      - "/home/scncoreorion/server/files:/app/www/private/files"
      - "/home/scncoreorion/server/logs:/var/0x101 Cyber Security/scncoreorion RMM/Server/"
      - "/home/scncoreorion/letsencrypt:/app/letsencrypt"
      - /etc/localtime:/etc/localtime:ro
    ports:
      - "7080:7080"
...
echo "docker-compose.yml created in /home/scncoreorion"

echo ""

read -p "Everything seems to be ready. Startup scncoreorion RMM containers now? (Y/n): " start_now
start_now=${start_now:-Y}

if [[ "$start_now" =~ ^[Yy]$ ]]; then
    echo "Starting scncoreorion containers..."
    sudo docker compose -f /home/scncoreorion/docker-compose.yml up -d
else
    echo "You can start it later with:"
    echo "   sudo docker compose -f /home/scncoreorion/docker-compose.yml up -d"
fi
...
echo "   sudo docker logs scncoreorion-web-console"
echo "   sudo docker logs scncoreorion-rmm-server"
echo ""
...
echo ""
echo "Thank you for using scncoreorion RMM!"
